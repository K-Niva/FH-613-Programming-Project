...
def upload_file():
    """Handle file upload with immediate email validation."""
    ...
    try:
        ...
        if is_scheduled and start_date and end_date and process_time:
            ...
            schedule_item = {
                'job_id': schedule_id, 'job_type': 'SCHEDULE_TEMPLATE', 'job_status': 'ACTIVE',
                'recipient_email': email, 'original_filename': filename, 's3_key': s3_key,
                'schedule_start_date': start_date, 'schedule_end_date': end_date,
                'schedule_process_time': process_time, 'upload_time': dt.datetime.utcnow().isoformat()
            }
            progress_table.put_item(Item=schedule_item)

            initial_run_id = str(uuid.uuid4())
            run_item = {
                'job_id': initial_run_id, 'job_type': 'JOB_RUN', 'job_status': 'PENDING',
                'original_filename': filename, 'recipient_email': email,
                'parent_schedule_id': schedule_id, 'upload_time': dt.datetime.utcnow().isoformat()
            }
            progress_table.put_item(Item=run_item)
            s3_client.copy_object(
                Bucket=S3_BUCKET_NAME, Key=s3_key, CopySource={'Bucket': S3_BUCKET_NAME, 'Key': s3_key},
                Metadata={'recipient-email': email, 'job-id': initial_run_id}, MetadataDirective='REPLACE'
            )
            ...
            item = {
                'job_id': on_demand_job_id, 'job_type': 'JOB_RUN', 'job_status': 'PENDING',
                'original_filename': filename, 'recipient_email': email,
            ...
            s3_client.copy_object(
                Bucket=S3_BUCKET_NAME, Key=s3_key, CopySource={'Bucket': S3_BUCKET_NAME, 'Key': s3_key},
                Metadata={'recipient-email': email, 'job-id': on_demand_job_id}, MetadataDirective='REPLACE'
            )
            job_id_for_polling = on_demand_job_id
            message = 'File upload successful. Processing has started immediately.'

        return jsonify({'success': True, 'message': message, 'job_id': job_id_for_polling}), 200
